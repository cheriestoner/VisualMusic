...
public class MappingOSC : MonoBehaviour {
    ...
    [ReadOnly] public float onsetOSC;
    ...
    void Start()
    {
        ...
        osc.SetAddressHandler("/onsetTime", OnReceiveOnset);
        InitPSSettings(); 
    ...
    void OnReceiveOnset(OscMessage message)
    {
        if(isSinging)
        {
            onsetOSC = message.GetFloat(0);
            if(onsetCount == 0) onsetCount = 1;
            else onsetCount += 1;
        }
    }
    ...
    void InitPSSettings()
    {
        ...
        ParticleSystem.MinMaxCurve speedCurve = new ParticleSystem.MinMaxCurve(1.0f, Methods.GetCurve(1f, 0.5f));
    ...
    void Update()
    {
        if(!isSinging)
        {
            ...
            if(m_system.isPlaying) ResetVisualizer();
        }
    }
    ...
    IEnumerator ParticleVisualizer() //float energy, float brightness
    {
        ...
        while(isSinging)
        {
            if(!m_system.isPlaying) m_system.Play();
            ...
            // pitchScale = 1f - Methods.LogMap(GetComponent<PitchAmplitude>().notePitch, 45f, 80f);
            pitchScale = 1f - Methods.LogMap(pitchOSC, 45f, 80f);
            ...
            size.enabled = true;
            size.size = new ParticleSystem.MinMaxCurve(Mathf.Lerp(0.1f, 5f, shapeScale), Methods.GetCurve(1f, 1f));
            if(stageNumber == 0) {
                if(!pitchDetected) emission.rateOverTime = 0;
                else emission.rateOverTime = 50f;
                yield return null;
                continue;
            }
            
            main.startColor = Color.HSVToRGB(0, 0, centroidScale);
            if(pitchDetected) emission.rateOverTime = Mathf.Lerp(10, 50, 1 - shapeScale);
    ...
    IEnumerator TempoTracker()
    {
        ...
        while(isSinging)
        {
            ...
            // int colorIndex;
            // if(tempo/2 <= 1) colorIndex = 1;
            // else if(tempo/2 >= 5) colorIndex = 5;
            // else colorIndex = (int)(tempo/2);
            ...
            if(beatBuffer.GetMovingAverage() >= 0) {
                ...
                // color background
                int colorIndex;
                if(tempo <= 1) colorIndex = 0;
                else if(tempo >= 5) colorIndex = 4;
                else colorIndex = (int)tempo - 1;
                if(stageNumber == 2) colorIndex = 4 - colorIndex;
                StartCoroutine(ColorTransition(Methods.keyColors[colorIndex], 0.5f));
            }
        }
        beatBuffer.Clear();
        tempoTracking = false;
